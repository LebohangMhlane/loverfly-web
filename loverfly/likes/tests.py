...
class LikeTests(APISetupTests, TestCase):
    ...
    def test_like_unlike_a_post(self):
        ...
        # this post should already be liked by us:
        likers = Liker.objects.filter(post__id=1)
        self.assertEqual(len(likers), 1)

        # unlike the post:
        url = reverse("like_post", kwargs={"post_id": 1, "post_liked": "true"})
        ...
        # the post should have no likes:
        post = Post.objects.get(id=1)
        self.assertEqual(post.likes, 0)

        # the post should now be unliked:
        likers = Liker.objects.filter(post__id=1)
        self.assertEqual(len(likers), 0)

        # like the post again:
        url = reverse("like_post", kwargs={
                      "post_id": 1, "post_liked": "false"})
        api_response = self.client.get(
            self.LOCAL_HOST + url,
            HTTP_AUTHORIZATION="Token " + self.login_response["token"],
        )
        ...
        # this post should be liked by us again:
        likers = Liker.objects.filter(post__id=1)
        self.assertEqual(len(likers), 1)
